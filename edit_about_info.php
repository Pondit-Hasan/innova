  ...
  <div class="row">
    <div class="col-lg-12 col-sm-12 m-auto py-3 my-2">
      <div class="bg-success text-white">
        <h2 class="text-center py-3 bg_header">Welcome to About Section Page!</h2>
      ...
      <!-- update form -->
      <form action="update_about.php" method="post" class="px-3 border border-info bg-light text-dark" enctype="multipart/form-data" >
        <div class="form-group my-3">
          <input type="hidden" name="id" class="form-control" value="<?php echo $afer_assoc['id']; ?>">
        </div>
        <div class="form-group my-3 row">
          <label class="col-sm-2 col-form-label" for="about_title">about Title</label>
          <div class="col-sm-10">
            <input type="text" id="about_title" name="about_title" class="form-control" value="<?php echo $afer_assoc['about_title']; ?>">
          </div>
        </div>
        <div class="form-group my-3 row">
          <label class="col-sm-2 col-form-label" for="about_desp">about Description</label>
          <div class="col-sm-10">
            <textarea id="about_desp" type="text" name="about_desp" class="form-control" rows="5"><?php echo $afer_assoc['about_desp']; ?></textarea>
          </div>
        </div>
        <div class="form-group my-3 row">
          <label class="col-sm-2 col-form-label" for="gender">about sub title</label>
          <div class="col-sm-10">
            <input type="text" name="about_sub_title" class="form-control" value="<?php echo $afer_assoc['about_sub_title']; ?>">
          </div>
        </div>
        <div class="form-group my-3 row">
          <label class="col-sm-2 col-form-label" for="gender">about features</label>
          <div class="col-sm-10">
            <input type="text" name="about_features" class="form-control" value="<?php echo $afer_assoc['about_features']; ?>">
          </div>
        </div>
        <div class="form-group my-3 row">
          <label for="role" class="col-sm-2 col-form-label">about Status</label>
          <div class="col-sm-10">
            <select id="role" class="form-control" name="about_status">
              <option value=0>Select about Status</option>
              <option value=1 <?= ($afer_assoc['about_status']==1) ? 'selected' : ''; ?>>Active</option>
              <option value=2 <?= ($afer_assoc['about_status']==2) ? 'selected' : ''; ?>>Deactive</option>
            </select>
          </div>
        </div>
        <div class="custom-file">
          <input name="about_img" type="file" class="custom-file-input" id="customFile">
        ...
        <div class="from-group my-3">
          <div>
            <p>Present Image </p>
            <img class="md_thumbnail_img" src="uploads/abouts/<?php echo $afer_assoc['about_img']; ?>" alt="photo_<?php echo $afer_assoc['about_img']; ?>">
          </div>
        </div>
        <div class="form-group pt-2 text-center my-3">
          <a  class="btn btn-warning text-white" href="about_info.php">Don't Update</a>
